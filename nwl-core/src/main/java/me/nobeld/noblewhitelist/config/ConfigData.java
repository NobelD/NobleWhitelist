...
public class ConfigData {
    ...
    public void refreshData() {
        ...
        if (get(ServerCF.configVersion) <= 3) {
            boolean ar = configFile().get("whitelist.skip-name-change", false);
            set(SkipCF.skipName, ar);
            configFile().remove("whitelist.skip-name-change");

            boolean op = configFile().get("whitelist.only-op-as-permission", false);
            set(ByPassCF.onlyOpPerm, op);
            configFile().remove("whitelist.only-op-as-permission");

            int pm = configFile().get("whitelist.permission-minimum-number", -1);
            set(ByPassCF.permissionMinimum, pm);
            configFile().remove("whitelist.permission-minimum-number");

            set(ServerCF.configVersion, 4);
        }
    ...
    public static class SkipCF {
        public static final ConfigContainer<Boolean> skipName = new ConfigContainer<>("skip.skip-name-change", false);
        public static final ConfigContainer<Boolean> skipUUID = new ConfigContainer<>("skip.skip-uuid-save", false);
    }

    public static class ByPassCF {

        public static final ConfigContainer<Boolean> onlyOpPerm = new ConfigContainer<>("bypass.only-op-as-permission", false);
        public static final ConfigContainer<Integer> permissionMinimum = new ConfigContainer<>("bypass.permission-minimum-number", -1);
    }
