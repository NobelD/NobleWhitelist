...
public class MessageData {
    ...
    public static Component playerAbout(WhitelistEntry data) {
        if (data.isSaved())
            return AdventureUtil.formatAll("<prefix><#FF9CBB>This user was found: <#B490F0>- <#99EAFE>Index<#65A8FF>: <#F07DF0>" + data.getRowId());
        else return AdventureUtil.formatAll("<prefix><#FF9CBB>This user was found:");
    }
    ...
    public static Component permissionInf1(NWLData data) {
        return AdventureUtil.formatAll("<prefix><#FBC36F>Only OP: <#FF6040>" + (data.getConfigD().get(ConfigData.ByPassCF.onlyOpPerm) ? "enabled" : "disabled"));
    }

    public static Component permissionInf2(NWLData data) {
        int min = data.getConfigD().get(ConfigData.ByPassCF.permissionMinimum);
    ...
    public static Component checkingAlready(CheckingType type, CheckingOption option) {
        return AdventureUtil.formatAll("<prefix><#FBC36F>The checking type of <#FF6040>'" + type.name() + "' <#FF6040>already has the value of <#FF6040>'" + option.name() + "'");
    }

    public static Component checkingChange(CheckingType type, CheckingOption option) {
        return AdventureUtil.formatAll("<prefix><#FBC36F>The checking type of <#FF6040>'" + type.name() + "' <#FF6040>was changed to <#FF6040>'" + option.name() + "'");
    }
}
