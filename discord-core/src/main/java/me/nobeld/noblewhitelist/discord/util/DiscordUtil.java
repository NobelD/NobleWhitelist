...
public class DiscordUtil {
    ...
    public static MessageCreateData getMessage(NWLDsData data, ConfigContainer<?> cont, @Nullable Map<String, String> placeholders) {
        ...
        switch (sec.get("type", "null").toLowerCase()) {
            ...
            case "mixed" -> msg = messageFromSection(sec, placeholders).addEmbeds(embedFromSection(data, sec, placeholders).build()).build();
    ...
    public static EmbedBuilder embedFromSection(@Nullable NWLDsData data, FlatFileSection section, @Nullable Map<String, String> placeholders) {
        ...
        if (!author.singleLayerKeySet().isEmpty()) {
            try {
                ...
                if (data != null) data.logger().log(Level.WARNING, "The author part of the message '" + section.getPathPrefix() + "' is not well formatted.");
            }
        }
        ...
        if (!body.singleLayerKeySet().isEmpty()) {
            try {
                ...
                if (data != null) data.logger().log(Level.WARNING, "The body part of the message '" + section.getPathPrefix() + "' is not well formatted.");
            }
        }
        ...
        if (!field.singleLayerKeySet().isEmpty()) {
            for (String f : field.singleLayerKeySet()) {
                try {
                    ...
                    if (data != null) data.logger().log(Level.WARNING, "The field '" + f + "' of the message '" + section.getPathPrefix() + "' is not well formatted.");
                }
            }
        }
        ...
        if (!image.singleLayerKeySet().isEmpty()) {
            try {
                ...
                if (data != null) data.logger().log(Level.WARNING, "The image part of the message '" + section.getPathPrefix() + "' is not well formatted.");
            }
        }
        ...
        if (!footer.singleLayerKeySet().isEmpty()) {
            try {
                ...
                if (data != null) data.logger().log(Level.WARNING, "The footer part of the message '" + section.getPathPrefix() + "' is not well formatted.");
            }
        }
